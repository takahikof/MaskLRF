...
### Create masking/reconstruction results ###
ckpts="../pretrain/MaskLRF/ckpt-last.pth"
exp_name_base="vis"
...
myarg_group_size=32
myarg_num_group=64
# myarg_num_group=32 # 可視化用に，トークン間のオーバーラップを減らすためにグループ数を減らす
...
# myarg_attn_target="random"

exp_name=$exp_name_base
CUDA_VISIBLE_DEVICES=$gpuid python -u main_vis.py --test --ckpts $ckpts --config $cfgfile --exp_name $exp_name --val_freq 20 \
