...
def fps_for_oriented_pointcloud(data, number):
    '''
        data B N 6 (each 3D point has 3D normal vector)
        number int
    '''
    fps_idx = pointnet2_utils.furthest_point_sample(data[:,:,0:3].contiguous(), number)
    fps_data = pointnet2_utils.gather_operation(data.transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous()
    return fps_data
