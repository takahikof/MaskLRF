...
# def random_mask( pc, scale_min=0.6, scale_max=1.0 ):
from sklearn import metrics
def random_mask( pc, num_group=64, group_size=32, mask_ratio=0.6, target_n_points=1024 ):
    # 複数個のパッチをマスク
    P = pc.shape[0]
    C = pc.shape[1]

    centers = farthest_point_sample( pc, num_group )

    D = metrics.pairwise_distances( centers, pc )
    idx = np.argsort( D )[:,0:group_size]
    np.random.shuffle(idx)

    n_visible_patch = int(num_group * (1.0-mask_ratio) )

    visible_point_idx = idx[0:n_visible_patch]
    visible_point_idx = visible_point_idx.reshape(-1)
    visible_point_idx = np.unique( visible_point_idx )

    if( visible_point_idx.size < target_n_points ): # 足りない場合は duplicate
        flag = True
        while( flag ):
            n = target_n_points - visible_point_idx.size
            np.random.shuffle( visible_point_idx )
            visible_point_idx = np.concatenate( [ visible_point_idx, visible_point_idx[ 0:n ] ] )
            if( visible_point_idx.size >= target_n_points ):
                flag = False
    elif( visible_point_idx.size > target_n_points ): # 多い場合は subsample
        np.random.shuffle( visible_point_idx )
        visible_point_idx = visible_point_idx[ 0:target_n_points ]

    pc = pc[ visible_point_idx ]
    return pc

def random_crop( pc, scale_min=0.6, scale_max=1.0, target_n_points=1024 ):
    # 1つの球で切り出す
    P = pc.shape[0]
    C = pc.shape[1]
    ...
    D = metrics.pairwise_distances( pc[ randint ].reshape(1,3), pc )
    visible_point_idx = np.argsort( D )[:,0:knn]
    visible_point_idx = visible_point_idx.reshape(knn)
...
class Modelnet40_DG(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # lbl = self.lbl_list[idx]
        # pc = np.load(self.pc_list[idx])[:, :3].astype(np.float32)
        # # mask_ratio = np.random.uniform( low=0.1, high=0.8 )
        # # pc = random_mask( pc, mask_ratio=mask_ratio, target_n_points=self.npoints )
        # # pc = random_crop( pc, scale_min=0.6, scale_max=1.0, target_n_points=self.npoints )
        # pc = random_crop( pc, scale_min=0.5, scale_max=0.75, target_n_points=self.npoints )
        # pc = normal_pc(pc) # マスク後に正規化する方が target domainと類似したデータが作れるはず
        # # np.savetxt( "pc_"+str(idx)+".xyz", pc )
        # # quit()
        # pt_idxs = np.arange(0, pc.shape[0])
        # if self.subset == 'train':
        #     np.random.shuffle(pt_idxs)
